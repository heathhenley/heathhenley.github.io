...
I can't give a *too* many of the details here, but we've switched to a two stage model that I trained using standard available PyTorch models. The first does object detection and predicts a bounding box for each region of interest (ROI), as well as the keypoints in there. And then we run a classifier on each cropped ROI. Should have been able to do this with a single model, but training was way easier with it split. I will probably try it again some time. The classifiers are fine-tuned on the specific species each region so that they perform much better then the generalized models that we know about. Some of those are publicly available and very cool - but fine-tuning on the 20 - 30 species that they really care about in the region is always better than a generalized model trying to get many 100's correct, at least as far as I've seen.

The "keypoint" detection is a pose estimation model trained on a bunch of manual annotations by researchers. Basically they label the start of the fish, then there are [different ways to record its length](https://en.wikipedia.org/wiki/Fish_measurement), so all those points are generated by the model (standard length, fork length, total length).
