...
Then this will depend on lot on the framework you're using - but we can create an endpoint in our app, `/auth/authorize` below, that will (1) make this url with the correct parameters, (2) generate some random state so store on the client and then finally actually trigger the redirect. For example in `FastAPI` it might look something like this: 
...
So that's it - you get back the user's info and Google has handled all the password storage / checking for you. It's then up to you for your specific case to figure out what you want to do with that information.
